...
import { CheckCircle, Lightbulb, XCircle, Zap } from "lucide-react";
...
export function CheckCard({ card, mode, onAnswer, isAnsweredCorrectly, attempts }: CheckCardProps) {
    ...
    return (
      <div className="flex flex-col h-full min-h-full px-6 py-8 bg-bg-primary">
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-text-secondary">You got this one correct. Swipe up to continue.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full min-h-full px-6 py-8 bg-bg-primary">
      ...
      <div className="flex items-center gap-2 mb-6">
        <div className="px-3 py-1 rounded-full bg-accent-gold/10 border border-accent-gold/20">
          <span className="text-sm font-medium text-accent-gold">Quick Check</span>
        </div>
        {!isAnsweredCorrectly && (
      ...
      {/* Question */}
      <h2 className="text-xl md:text-2xl font-medium text-white mb-8">{question}</h2>
