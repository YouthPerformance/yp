...
import { SessionStack, WeekGrid, SafetyNote } from '../components/wolf'
import { useOnboarding } from '../context/OnboardingContext'
import {
  generateWolfFirstMessage,
  WOLF_SUGGESTED_QUESTIONS,
  LANE_OUTPUTS,
} from '../config/interestPills'
...
function WolfChat() {
  ...
  const handleShowPlan = () => {
    ...
    const planMessage = {
      id: messages.length + 1,
      role: 'wolf',
      content: "Here's your 7-day plan. Start today, and by the end of the week you'll feel the difference.",
  ...
  const getWolfResponse = (questionId) => {
    const responses = {
      ...
      frequency: {
        content: "3 times per week is the sweet spot. That's enough to build the habit without overdoing it. On off days, a quick 2-minute micro session keeps the pattern fresh.",
        blocks: [],
      },
      week1: {
        content: "Here's a safe Week 1 plan based on your goals. We start gentle and build from there.",
        blocks: [{ type: 'plan', data: plan }],
      },
      pain: {
        content: "If there's any sharp pain or limping, stop immediately. That's the rule. Mild muscle soreness the next day is normal — that's your feet waking up. Sharp pain during an exercise means stop, rest, and try again tomorrow. No pushing through.",
        blocks: [{ type: 'safety', variant: 'warning' }],
      },
      progress: {
        content: "Look for these signs: Quieter landings during sports. Better balance on one leg. Less ankle rolling. Most parents notice a difference in 7-14 days of consistent practice. The key is 'consistent' — short sessions you repeat beat long sessions you skip.",
        blocks: [],
      },
    }
              ...
              >
                ...
                {message.blocks?.map((block, idx) => (
                  <div key={idx} className="mt-3">
                    {block.type === 'stack' && (
                      <SessionStack stack={block.data} />
                    )}
                    {block.type === 'plan' && (
                      <WeekGrid plan={block.data} currentDay={1} />
                    )}
                    {block.type === 'safety' && (
                      <SafetyNote
            ...
            <div className="flex justify-start">
              <div className="bg-black-200 rounded-2xl rounded-bl-sm p-4">
                <div className="flex gap-1">
                  <span className="w-2 h-2 bg-dark-text-tertiary rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                  <span className="w-2 h-2 bg-dark-text-tertiary rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                  <span className="w-2 h-2 bg-dark-text-tertiary rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                </div>
              </div>
            </div>
          )}
        ...
        <div className="max-w-2xl mx-auto">
          {!showPlan && messages.length > 0 && (
            <Button
              ...
              fullWidth
              className="mb-3 shadow-glow-cyan"
            >
