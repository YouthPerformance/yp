...
export function RadioCard({ value, children, icon, disabled = false, compact = false }) {
  ...
  return (
    ...
    >
      ...
      {icon && (
        <div
          className={`${compact ? "text-xl" : "text-2xl"} ${isSelected ? "text-cyan-500" : "text-dark-text-secondary"}`}
        >
          {icon}
        </div>
      )}
