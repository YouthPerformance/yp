  ...
  return (
    ...
    `}>
      <div className="flex items-start gap-2">
        ...
        <div>
          {title && (
            <p className={`font-medium text-white ${compact ? 'text-sm' : ''} mb-1`}>
              {title}
            </p>
          )}
...
export const SAFETY_NOTES = {
  ...
  returnCarefully: (
    <SafetyNote type="info" title="Coming Back Carefully">
      Start gentle. Build tolerance the right way. If something feels off, rest and try again tomorrow.
    </SafetyNote>
  ),
  quietLanding: (
    <SafetyNote type="info" title="Quiet Landings">
      Focus on landing soft and quiet. If you hear a loud thump, slow down and absorb more with your legs.
    </SafetyNote>
  ),
  noJumps: (
